...
//        public static bool DefaultReverse = false;
        ...
        {
            get { return DefaultReversePrivate[0]; }
            set { DefaultReversePrivate[0] = value; }
        }

        private static readonly bool[] DefaultReversePrivate = {false};
...
//		}
        ...
        */
        ...
        private DerObjectIdentifier DecodeOid(string name, IDictionary lookUp)
        {
            ...
            var oid = (DerObjectIdentifier) lookUp[name.ToLowerInvariant()];
            if (oid == null)
            {
...
//				this.added.Reverse();
        ...
        */
        ...
        private void AppendValue(StringBuilder buf, IDictionary oidSymbols, DerObjectIdentifier oid, string val)
        {
            ...
            while (index != end)
            {
                if ((buf[index] == ',') || (buf[index] == '"') || (buf[index] == '\\') || (buf[index] == '+') || (buf[index] == '=') || (buf[index] == '<') ||
                    (buf[index] == '>') || (buf[index] == ';'))
                {
                    buf.Insert(index++, "\\");
                    end++;
